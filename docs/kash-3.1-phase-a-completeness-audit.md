...
| ----------- | ----------------------------------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T1 / MH-1…9 | Morning hand-off full ritual (right-side sheet) | **built** | `MorningHandoffModal` + `RitualSheet` (D36): opener, carryover keep/drop, recurring, project-tasks-today, composer + project search, goal offer, Top 3 from assembled list, hold preview (D15), Begin day / Skip. Runner: `MorningHandoffRunner.tsx`. |
| T2 / SC-1   | Interactive gap rows (Walk / Breathe)           | **built** | `SelfCareGapRow.tsx` in `TimelinePane` — one-tap walk timer + breathe.                                                                                                                                                                                |
| T2 / SC-2   | Walk reminders (2–3/day, arbiter)               | **built** | `evaluateSelfCareWalkReminders` in `run-nudge-evaluation.ts`.                                                                                                                                                                                         |
...
| C3  | Evidence surfacing on large-goal completion | **built** | `evidence_surface` nudge: "Here's the trail…".                                          |
| C4  | Care stats facet split                      | **built** | `facetFrequencies` in `buildCareStatsSummary` / `CareStats`.                            |
...
| A2  | About-me in RDM / week-draft / nudge text | **gap (S)** | `fetchAboutMeContextBlock` only in chat assemble + EoD reflection on main. PR12 injects into week-draft / nudge / RDM path.                                                                                                                                                                                                                                  |
...
| Row                            | Status      | Notes                                                                          |
| ------------------------------ | ----------- | ------------------------------------------------------------------------------ |
| Global tokens (D9/D10/D12/D13) | **built**   | `--canvas`, category chrome, cutoff rule.                                      |
| Today (V3/V6/V8)               | **built**   | Progressive disclosure, slim timeline rail, micro-cleanups, onboarding (#155). |
...
1. **A1** — Companion can discuss plan changes but cannot propose most write tools (edit/delete/top-3/protected blocks/projects/phases). Data and chat exist; action catalog incomplete on main.
2. **A2** — About-me is stored and used in chat + EoD reflection only; RDM narration, week-draft, and nudge text generation do not inject it on main.
...
| -------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------- |
| A1 (+ A2, A3, A5 on same branch) | L (bundle) | [#156](https://github.com/Frontenac-studios/Flowstate/issues/156) — land `feat/kash-3.1-pr12-ai-companion` |
