...
- **`leads`** (`org_shared`) — a scored research artifact, **not** a project (avoids ~5 throwaway
  projects/week). Fields: `id, user_id, org_id, company_name, segment_id, source (sourced|manual|intake),
  score (0–100), confidence (0–100), rank, rationale (jsonb: fit/risk/strategy + gaps), state
  (new|contacted|engaged|proposal|promoted|dismissed|snoozed), dismiss_reason, snooze_until, run_id
  (nullable), project_id (nullable → set on promote), created_at, updated_at`.
