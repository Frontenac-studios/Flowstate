...
| Wellness                 | `care_activities`, `care_events`, `care_reflections`, `care_enums`, `daily_wins`, `user_values`                                                  | SHIPPED        | **`care_*` parked (§3.2); `daily_wins`, `user_values` cut**     |
...
Each of these is finished, harmless, and plausibly wanted back.
...
| **Care / garden** (`care` router, `care_activities`, `care_events`, `care_reflections`, `care_enums`, `/care`, `CareView` + ~18 components) | none          | Finished and harmless. The mission cuts it, but the owner's call (§8a #2, reaffirmed §8c) is to leave the door ajar rather than destroy the written-reflection substrate. Hosted holds **0 care rows**.                | Practice log + reflections (0 rows today).        |
...
| Care        | Remove the Care entry from nav (`NAV_GROUP_REFLECT_PLAN`) and the command palette.                                    | `src/app/(app)/care/page.tsx` → `notFound()` when `!FLAGS.care`.                                                | `CareView` never mounts (the route 404s).                                                                                     | Keep the `care` router registered (RLS-guarded); unreachable from the client.                             |

Remove `NAV_GROUP_REFLECT_PLAN` entirely once Plan is rebuilt (Care is parked, so its nav entry is already gone); the nav
...
> **Care was reclassified from KILL to PARK on 2026-08-21** (§8a #2, reaffirmed §8c),
> so it is not in the table below. It stays flagged off behind `FLAGS.care` with its
> route, `care` router, schema, and components intact; see §3.2. Hosted holds 0 care
> rows, so there is nothing to export. Everything else in this table is deleted.

| Item                                                                                                                                                  | Serves         | Reason                                                                                                                                                     | Data lost                         | Export first?                                                                                |
| ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| **Daily Wins** — router, `daily_wins`, body/mind/soul facets                                                                                          | none           | Named cut ("habit tracker").                                                                                                                               | Win history.                      | Optional; small dataset.                                                                     |
...
- [ ] About-me export written to `~/flowstate-exports/` and verified readable **before** the drop migration runs. (Care was parked, not dropped — 0 hosted rows, so no export; §8a.)
...
1. **Killing Care rather than parking it.** Product law 5 says park only what's finished and
   ...
   _(Resolved 2026-08-21: **PARK**, not kill — §8a #2, reaffirmed §8c; §3.3 updated to match. There were 0 hosted rows to export.)_
