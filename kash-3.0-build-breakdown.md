...
| §14 Task & Data Model | ✅            | ✅ `data-spine-build-spec.md` | ⬛ mostly | **re-baselined Jun 25:** P1 (category) ✅, P2 (time-tracking) 🟡 substantially built, P3 (dependencies) ✅ built (revised many-blockers design). **P4 (recurrence) 🏗️ in progress** — 4A schema+RLS committed; migration + 4B–4G pending local env (Node 20 / `npm install rrule`)                                                                                                                                               |
...
| §7 Week               | ✅            | 🟡                            | 🟡        | open Qs closed Jun 25 (tally-on-demand; learned over-commit threshold w/ §11 drift-flag, protected counts fully; protected recurrence = template+weekly-confirm; timeless protected = all-day pinned chip on Today; EoW review = soft Sun-eve nudge). **Build-spec TODO: add `protected_blocks` data model.** Build protected blocks, per-category load, weekly review; layout = horizontal-scroll days, gray week + white today |
