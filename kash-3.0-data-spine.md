...
- **`lastUsed` update rule** — always set `last_used_category` to the **resolved** category on successful create (whatever layer produced it), so habit stays true even when AI picked the first one.
