...
- **VF-2 — Lens state engine + controls** (VF2/VF3): one group lens + ≤2 active lenses (hard cap, 3rd drops oldest); UI control bar + keystroke.
- **VF-3 — Filter + group application**: pure helpers to filter/sort/group a task array by the active lens; group headers; wired into a surface.
- **VF-4 — Per-page defaults + rollout** (VF5 + project auto-ramp-by-phase color): Today clean · This Week day-groups + category-lens balance · Inbox clean flat + triage lenses · Projects Miller-as-is + phase-ramp + priority lens on demand.
