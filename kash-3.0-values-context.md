...
| #    | Decision               | Choice                                                                                         |
| ---- | ---------------------- | ---------------------------------------------------------------------------------------------- |
| V1-1 | Values: ranked vs flat | **Flat set** (no hierarchy)                                                                    |
...
| V2-3 | Edit retroactivity     | **Future only** — edits apply going forward; past/current untouched                            |
...
- **Used three ways:**
  1. **Tag** — goals (PM1-2) and optionally tasks reference a value.
  2. **AI leans on them** — the AI weights value-aligned suggestions and **explains** why
     ("suggested — it serves Health, one of your values").
...
- **Content source:** **both** — you author freely, and the AI **proposes** additions it learns, shown
  as ghosted suggestions you accept/reject (reuses the §9 ghosted-accept pattern; per-item, staged).
...
- Value link: `goals.value_id` (already from PM1-2, nullable); optional `tasks.value_id`.
...
- AI persona, prompt text, and exactly _how_ the AI weighs values vs urgency → **§11**.
