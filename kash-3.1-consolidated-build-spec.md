...
| D17 | Project-mode calendar hues        | **Rotate the existing Apple palette** (the five category solids + reserved yellow) — no new palette. **Supersedes** the "project hues must not collide with category hexes" build-spec flag (the multi-project-calendar coloring note in `kash-3.0-plan.md` §9 — inline, not a numbered §9.2).                                                                                                                                                                                                                                                            |
...
| D19 | Week grid on the tinted canvas    | ⚠️ **Reversed by D40 (Jul 7).** ~~All-white day columns, marked today — every day is a white card on the `--canvas` tint; today gets an ink border + black date pill.~~ Superseded: the Jun-24 inverted emphasis is restored (see D40). Horizontal-scroll days stay.                                                                                                                                                                                                                                                                                      |
...
| D27 | Backlog identity                  | **Light list, dark sky** — the view carries the theme: List renders on the D9 light system (practical staging surface); switching to Sky goes dark and immersive (stargaze mode, brightened category hues). Theme toggle retired. **Supersedes `backlog-reframe-build-spec` BK3** — Sky is **kept** (not dropped); the Themes lens is **additive** alongside it.                                                                                                                                                                                          |
...
| D39 | Miller columns source-of-truth    | **`kash-3.0-projects-miller.md` is the source of truth for the Miller columns view** (reverses the earlier "ratify current build" take). Depth is **unlimited** (MC-1's two-level cap voided in that doc). **Phase detail is inline expansion** in place, not a child-column header (ID-3/PROJ-C resolved as inline) — matches the current build. The collapsible "Completed · n" group per column (PROJ-A) is **built** (PR #178). MC-1/MC-2/ID-1, pre-drill, and blank-state ghost columns already match. Full detail in V10.                           |
| D40 | Week today-marker (reverses D19)  | **Jun-24 inverted emphasis restored** (Jul 7) — soft-gray day columns (`--active-surface`), **today the lone white column + white date pill** (hairline, not the black pill); contrast carries the signal, no ink border. **Reverses D19** (all-white cards / ink-bordered today) and **amends D20/V9** accordingly; the canvas-tint field is kept behind the columns. Deliberate **local exception to D9** (white-cards-on-canvas) for This Week only. Source: `kash-3.0-visual-redesign.md` (Jun 24). Code: `WeekColumn.tsx`, `ColumnTallyPopover.tsx`. |
...
> columns — `kash-3.0-projects-miller.md` set as source of truth (Jul 7). Fixed-width columns
> (PR #178) confirmed correct; the "Completed · n" group is **built** (PR #178). MC-1 depth cap
> removed (unlimited nesting) and phase detail resolved as inline expansion (ID-3/PROJ-C) — both
> match the current build; pre-drill and blank-state ghost columns kept. D40: This Week today-marker (Jul 7) — **reverses D19**, restoring
> the Jun-24 inverted emphasis (soft-gray day columns, lone white today column + white date pill);
> amends D20/V9; a deliberate local exception to D9 for This Week only.
...
> **⚠️ Status unclear — Bingo → Goals IA rename is half-built; revisit.** This track covers goal
> _steering_ only. The separate **IA reframe** (`goals-view-build-spec` G5 + `backlog-reframe-build-spec`)
> — "coordinated sweep: rename the Bingo horizon → Goals, Bingo becomes a view mode, add List/Journey
> lens" — was never given a D-number and is **only partially landed**: the `"goals"` horizon is live in
> `PlanHorizonView`, but `bingo_cards`/`bingoCards`/`getOrCreateBingoCard` and the whole
> `src/components/kash/plan/bingo/` folder (incl. `GoalJourneyTimeline.tsx`) are still present. Decide
> whether to finish the rename or abandon it; until then it is **untracked drift**, not a closed item.
...
- **V9 This Week pass (D19–D21 · D19 reversed by D40) · M:** `WeekCanvas`/`WeekColumn` — **as built
  per D40 (Jun-24 inverted emphasis):** day columns render **soft-gray** (`--active-surface`) on the
  `--canvas` tint, and **today is the lone white column** (`--surface`) + a **white date pill**
  (hairline, weekday label inverted); contrast comes from the white-vs-gray columns, **not** an ink
  border. Horizontal-scroll days (4-of-7 viewport) stay. _(D40 reverses D19's earlier all-white-cards
  / ink-bordered-today treatment; a deliberate local exception to D9 for This Week.)_ Each `ColumnTallyPopover` header
...
- **V10 Projects pass (D23–D25, D39) · M:** `MillerColumnsView` — **follows
  `kash-3.0-projects-miller.md` (source of truth, D39).** Drop the outer strip card; each level
  renders as a white card directly on the `--canvas` tint with a **tight gap** (`--gap-sm`, not
  section-scale spacing); active level gets the 1px ink border; **columns are a fixed 256px (`w-64`)
  equal width, never stretch** (MC-2; shipped PR #178), so a shallow tree leaves open canvas to the
  right and the strip keeps `overflow-x-auto` so deeper trees scroll (D13 peek + fade).
  **Recursion has no depth cap** (MC-1 as corrected, D39): each phase opens one more column.
  **Selecting a phase expands an inline phase-detail panel in place** (progress / rename / dates;
  ID-3 / PROJ-C resolved as inline, not a column header) **and** opens the phase's contents in the
  next column; **selecting a task likewise expands the row in place, inline-editable** (ID-1).
  **Completed items sink to a per-column collapsible "Completed · n" group** (PROJ-A). Pre-drill down the first-incomplete path on open, and dashed
  ghost placeholder columns on the blank state (beside the D12 invitation), are both kept (D39).
  Drop-target outlines appear only during an active drag. `ProjectCard` — "No tasks yet" renders a faint
...
| This Week                                            | ⬛ built Jul 2 (#142) · today-marker reverted to Jun-24 inverted per D40 (Jul 7)                           | D19→D40 D20 D21 (V9) + W1 D5        |
| Projects (index + detail)                            | ⬛ built Jul 2 (#143) · D39 "Completed · n" group built (#178); phase-detail stays inline (#179)           | D23 D24 D25 D26 D17 P4 P5 D39 (V10) |
...
- **Out-of-scope infra (tracked elsewhere, intentionally not in the last-mile tracks):** offline-first
  optimistic UI + sync-state animations live in `kash-3.0-backend-optimization-spec.md` (D1) and
  `kash-3.0-animation-sweep.md` (AN-B7/AN-B8). Built infra, not 3.1 last-mile — noted here only so it
  isn't mistaken for dropped planning.
