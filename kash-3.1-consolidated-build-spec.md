...
> Overlays design pass (audit/spec session; per-option mockups); build detail V14.
...
| F4  | `glass.css` teardown once alias audit is clean                                                                                                                                                                                                                                                                                                                                                                                                                     | S    |
| F5  | Verify D2 sync detail panel expands from the footer dot with pending count / manual sync / conflicts                                                                                                                                                                                                                                                                                                                                                               | S    |
| F6  | Status-doc truth pass: update `docs/build-status.md` + `kash-3.0-goals-vs-build.md` scorecard to match code (auto-DND, morning hand-off, BK3–BK5) and point both at this doc; also amend §9's calendar-drag wording per D22                                                                                                                                                                                                                                        | S    |
...
- **V10 Projects pass (D23–D25) · M:** `MillerColumnsView` — drop the outer strip card; each
  ...
  not section-scale spacing); active level gets the 1px ink border; ghost placeholder columns
  removed — empty slots are open canvas, and drop-target outlines appear only during an active
  drag; the strip keeps `overflow-x-auto` so deeper trees horizontally scroll, with the D13
...
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Global tokens (canvas, category chrome, cutoff rule) | ✅ decided Jul 2                                                                                           | D9 D10 D12 D13                   |
| Today                                                | ✅ decided Jul 2                                                                                           | D11 D14 (V3/V6, not built)       |
| This Week                                            | ✅ decided Jul 2                                                                                           | D19 D20 D21 (V9, not built)      |
