...
DROP POLICY IF EXISTS "focus_blocks_select_own" ON focus_blocks;
CREATE POLICY "focus_blocks_select_own" ON focus_blocks FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "focus_blocks_insert_own" ON focus_blocks;
...
DROP POLICY IF EXISTS "focus_blocks_update_own" ON focus_blocks;
CREATE POLICY "focus_blocks_update_own" ON focus_blocks FOR UPDATE USING (user_id = auth.uid());

DROP POLICY IF EXISTS "focus_blocks_delete_own" ON focus_blocks;
